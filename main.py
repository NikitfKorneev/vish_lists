...
@app.get("/calendar/holiday/{holiday_id}/edit-wishlists", response_class=HTMLResponse)
async def edit_holiday_wishlists(request: Request, holiday_id: str):
    user = get_current_user(request)
    if not user:
        return RedirectResponse("/login")

    # Получаем праздник
    holiday = supabase.table("holidays")\
    ...
    if not holiday.data:
        raise HTTPException(404, "Праздник не найден или не ваш")

    # Все вишлисты пользователя
    wishlists = supabase.table("wishlists")\
        .select("id, title")\
        .eq("user_id", user.id)\
        .execute()

    # Текущие привязанные вишлисты
    current_links = supabase.table("holiday_wishlists")\
        .select("wishlist_id")\
        .eq("holiday_id", holiday_id)\
        .execute()

    current_ids = {link["wishlist_id"] for link in current_links.data or []}

    return templates.TemplateResponse("edit_holiday_wishlists.html", {
        "request": request,
...
@app.post("/calendar/holiday/{holiday_id}/edit-wishlists")
async def update_holiday_wishlists(request: Request, holiday_id: str, wishlist_ids: list[str] = Form(None)):
    user = get_current_user(request)
    if not user:
        raise HTTPException(401)

    # Проверяем, что праздник принадлежит пользователю
    holiday = supabase.table("holidays")\
        .select("user_id")\
        .eq("id", holiday_id)\
        .eq("user_id", user.id)\
    ...
    supabase.table("holiday_wishlists")\
        .delete()\
        .eq("holiday_id", holiday_id)\
        .execute()
